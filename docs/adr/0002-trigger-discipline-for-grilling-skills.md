...
- **Eval expansion — LANDED (2026-08-24).** `trigger-vocabulary-prompt.yaml` and `no-trigger-business-pricing.yaml` were added to `domain-grilling/evals/.../tasks/`, and the no-trigger threshold on `grilling`'s `no-trigger-terminology-decision.yaml` was lowered from 0.4 to 0.5. The wording change is now covered by Skill Evaluator tasks in both directions. Remaining gap: those tasks have not yet been executed in this environment (waza is Copilot-only here), so the routing flip is staged but not yet empirically confirmed by the Skill Evaluator.
- **GLOSSARY.md "domain model" term — LANDED.** `GLOSSARY.md` now defines `Domain model` (distinct from "general plan", "code implementation plan", and "spec/PRD"), matching the "Code implementation plan" entry from the original ADR 0002.
- **Open risk — empirical confirmation pending (2026-08-24).** ADR 0002's standing proposition is that a body-only discriminator has limited usefulness unless the YAML `description` is itself narrowed; the 2026-08-24 change satisfies this by narrowing `grilling`'s description to strategy/direction/priority decisions and explicitly excluding domain-language cases. The remaining gap is purely verification: the Skill Evaluator tasks are staged but have not been run in this environment (waza is Copilot-only here), so the routing flip is not yet empirically confirmed. Secondary, lower-severity note (not an ADR 0002 core concern): the `grilling` YAML now uses a "Do not use when…" negation rather than a positive pointer; the 2026-08-20 Follow-up treated such negations as weaker signals per GLOSSARY.md § Negation, but the description is nonetheless narrowed, so this is a signal-strength nuance rather than the ADR 0002 failure mode.

## Status note (2026-09-06)

The `grilling` skill was removed and consolidated into `technical-grilling` (see ADR 0004). This ADR's lessons on YAML-craft trigger discipline remain valid and are not superseded.
