...
4. Verify the CLI is installed by checking if it is available in `PATH`. If not found, follow the no-auto-install flow defined in the loaded reference. The agent shall not install host CLIs under any circumstance; the user is responsible for installing the CLI on their own machine and the agent shall point the user to the canonical install page URL.
