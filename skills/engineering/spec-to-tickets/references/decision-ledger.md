...
If the input is a `technical-grilling`
...
or, for a `technical-grilling` ledger:
...
1. **Pre-question append.** Before presenting a clarifying question
   to the user, append an `Ixxx` record to the input ledger with
   the `Prompt` field filled and the other three fields marked
   `TBD`, then bump the `<!-- next-i: Ixxx -->` sentinel. The `TBD`
...
2. **Post-response complete.** After the user answers, edit the same
   ...
   `Notes` with the user's exact words and the agent's notes.
   Read-back to confirm the four fields are now filled and the
   `Ixxx` is in its expected position in the file.
...
- `Prompt` is the **verbatim** agent text that was presented to the
  ...
  paraphrase the prompt. For a user-posed clarifying interaction,
  prefix the verbatim user question with `<user-posed>`; the agent's
  answer goes in `Resolution`.
