...
- The spec is incomplete, vague, or unresolved (check whether the user has access to a skill that creates specifications or requirements documents; recommend it if available, otherwise name no specific skill)
...
**Decision Ledger pairing.** If a Decision Ledger exists at `docs/decisions/DECISIONS-<repo>-<feature>.md` (produced by `technical-grilling`), read it alongside the spec. The ledger is the authoritative source for resolved functional (`Dxxx`) and technical (`Txxx`) decisions. Every ticket's acceptance criteria and constraints must cite a `Dxxx` or `Txxx` record using `filename#<Dxxx|Txxx>` format - paraphrase the ledger record (a `Dxxx` or `Txxx` entry in a Decision Ledger), never the spec's summary of it. A spec that ships without a ledger, or a ledger whose `Dxxx`/`Txxx` records are not all covered by at least one ticket, is a coverage gap to surface (not to silently fix). If no Decision Ledger is found, check whether the user has access to a skill that creates specifications or requirements documents; recommend it if available, otherwise name no specific skill. Make this recommendation before proceeding with ticket decomposition.

Print the criteria status (one line per criterion: met / missing) as part of the Step 3 output and list which `Dxxx`/`Txxx` records the proposed tickets would cover. Proceed to Step 4 if all four are met; the user can interject at any point in the conversation. If all four criteria are missing entirely, do not abort - check whether the user has access to a skill that creates specifications or requirements documents; recommend it if available, otherwise name no specific skill, to develop the missing input. If one or more criteria are missing (but not all four), abort and report which criteria are unsatisfied. Suggest checking whether the user has access to a skill that creates specifications or requirements documents; recommend it if available, otherwise name no specific skill.
...
**Decision Ledger and blueprint deletion prompt (when source files are present)** - when a Decision Ledger or implementation blueprint was consumed as input in Step 2, present a non-blocking post-creation prompt after the summary report, asking whether to delete the source files now that the tickets are published. The prompt covers the input Decision Ledger (`docs/decisions/DECISIONS-<repo>-<feature>.md` or whatever path the user named) and any companion implementation blueprint named in the same path family (for example, `docs/blueprints/BLUEPRINT-<repo>-<feature>.md`). The prompt is rendered as a separate `### Source-file cleanup` section after the coverage matrix; it offers three explicit options - delete both files now, delete the ledger only, or keep both files (defer indefinitely). The prompt does not block on a response - the user can decline - and the agent does not auto-delete. When no Decision Ledger or implementation blueprint was provided as input, this section is omitted entirely. Do not modify the input ledger records as part of the prompt; the `Ixxx` records appended during the session stay in the ledger if the user keeps the file.
...
- [ ] Step 7 asks the user before overwriting existing tickets.
...
- [ ] If the skill aborted in Step 3, the abort reason and the suggested next step (check whether the user has access to a skill that creates specifications or requirements documents) are surfaced in the output.
- [ ] The all-four-missing branch in Step 3 routes to a skill that creates specifications or requirements documents rather than aborting.
- [ ] The no-ledger branch in Step 3 continues without ledger I/O.
...
- [ ] PR count is resolved in Step 5 - the agent asks the user or reads from spec/supporting documentation, defaulting to 1 PR.
...
- [ ] The workflow runs in Collaborative mode - the user is in the loop at decision points and the skill pauses for input before proceeding.
- [ ] The agent never installs a host CLI (`gh`, `glab`, `tea`, `fj`, or any other). Host-CLI installation is always the user's responsibility. The agent points the user to the canonical install page URL from the hand-maintained table in `references/host-cli-detection.md` and waits for the user to install the CLI.
- [ ] The agent never fetches, scrapes, parses, or quotes remote README content, release pages, or install scripts to derive an install command for a host CLI. The canonical install page URL is taken from the static table in `references/host-cli-detection.md`; the user reads the page in their own browser.
- [ ] The agent never runs `curl | sh`, `iwr | iex`, `pip install`, `apt install`, `brew install`, `winget install`, `scoop install`, or any other install command for a host CLI.
- [ ] When a Decision Ledger is provided as input, the `references/decision-ledger.md` reference is loaded in Step 2 and the ledger file is read end-to-end before proceeding. The agent does not create a Decision Ledger of its own.
- [ ] When a Decision Ledger is provided as input, every clarifying question presented in Steps 3, 4, and 6 is preceded by a fresh `Ixxx` record appended to the input ledger (with `Prompt` filled, the other three fields set to `TBD`, and the `<!-- next-i: Ixxx -->` sentinel bumped). Each `Ixxx` is completed in place after the user answers; `Prompt` is never amended, records are never duplicated or moved, and re-asks produce a fresh `Ixxx` rather than an edit to the prior record.
- [ ] When a Decision Ledger or implementation blueprint was consumed as input, the Step 10 summary report includes a `### Source-file cleanup` section that offers three explicit options (delete both, delete ledger only, keep both). The prompt is non-blocking and the agent does not auto-delete. When no source files were provided, the section is omitted entirely.
