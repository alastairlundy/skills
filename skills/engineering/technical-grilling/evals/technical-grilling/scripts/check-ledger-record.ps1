...
#   5. The file ends with both sentinel comments:
#      `<!-- next-d: Dxxx -->` and `<!-- next-t: Txxx -->`.
#      (An unused legacy `<!-- next-i: -->` sentinel is tolerated.)
#
# Environment:
...
$dPattern = '### \[D(\d{3})\]'
$tPattern = '### \[T(\d{3})\]'
$sentinelPattern = '<!-- (next-[dt]): ([A-Z]\d{3}) -->'
...
foreach ($ledger in $ledgers) {
  ...
  # Check both sentinels exist (legacy third sentinel tolerated but not required)
  ...
  $hasAllSentinels = ($sentinelTypes -contains 'next-d') -and
                     ($sentinelTypes -contains 'next-t')

  if ($hasDriver -and $isHighest -and $hasAllSentinels) {
    $passing = $true
    $report = "PASS: $($ledger.Name) ends with $($lastPrefix)$lastId (highest of $totalRecords records, Driver field present, both sentinels present)"
    break
  }
}

if (-not $passing) {
  Write-Output 'FAIL: no ledger satisfied the post-pick criteria (>=2 Dxxx/Txxx records, last record has Driver, last record is highest in its stream, both sentinels present)'
  exit 1
}
