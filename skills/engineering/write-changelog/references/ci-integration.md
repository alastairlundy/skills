...
name: ci-integration
description: >-
  Canonical home for the marker vocabulary that write-changelog emits to the conversation so a CI wrapper can translate structured markers into process exit codes. Covers the empty-range abort, the defaulted marker, and the autonomous-mode overwrite-refusal marker.
license: MIT
---
...
Emitted in Step 4 when the ask-questions skill is unavailable or the user declines, and the sub-project section title defaults to `Global`.
...
### `overwrite-refused` (open follow-up)
