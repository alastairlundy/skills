...
2. **Collaborative Review**: Ask the user for confirmation in the agent's own words (e.g., *"Does this translation capture what you want the agent to do?"* - paraphrase freely rather than quoting a fixed script). The review question is a clarifying interaction. Before emitting it, append a fresh `Ixxx` record to the design ledger with `Prompt` set to the question text and the other three fields set to `TBD`. Interpret the user's natural-language reply as one of three outcomes: an affirmative (acceptance), a modification request, or a rejection.
   - On **acceptance** (any clear affirmative, e.g. "yes", "looks good", "accept as is"): complete the `Ixxx` in place with `User Response` = the user's reply (verbatim), the resolution (which branch the acceptance confirmed), and any non-load-bearing context. Append a fresh `Dxxx` record to the design ledger using the Dxxx record template (`Driver` = the underlying principle that motivated the translation, `Resolved Answer` = the user's affirmative reply, `Normalized Requirement` = the deterministic translation that was accepted, `Constraints` = any scope boundaries the user named). Bump the `<!-- next-d: Dxxx -->` sentinel atomically. Proceed to the next branch.
   - On **modification request**: complete the `Ixxx` in place with `User Response` = the user's reply (verbatim), and the resolution (the agent will re-ask for confirmation after applying the modifications). After applying the modifications, append a fresh `Ixxx` record for the re-ask with `Prompt` set to the re-ask question and the other three fields set to `TBD`. The accepted final response produces a `Dxxx` record on acceptance, not before.
   - On **rejection** (a clear "no" with no path forward): complete the `Ixxx` in place with `User Response` = the user's reply (verbatim), the user's rejection reasoning, and the resolution (the next-step choice the agent will surface). Append a `Dxxx` record only if the rejection resolved to a concrete decision (e.g., "exit the workflow with no further action"); otherwise the Ixxx itself is the durable record.
